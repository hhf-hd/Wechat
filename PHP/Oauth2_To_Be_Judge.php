...
//	var_dump($user_json);
	...
	setcookie("openid",$Openid,time()+3600*3);

	$con = mysql_connect("localhost","root","hhf150076");//连接数据库
	mysql_select_db("Student", $con);//选择数据库
	$Res = mysql_query("select User_OpenID from Judge_Data");//查询符合的Student_ID
	$Flag=1;
	while($raw = mysql_fetch_array($Res))
	{
			...
			//	setcookie("openid",$Openid,time()+3600*2);
				header("Location: http://www.personalwebhhf.cn/Wechat/To_Be_Judge.php?openid=$Openid");
				$Flag=0;
			}
	}
	if($Flag!=0)
